...
module.exports = {
    ...
    onStart: async function ({ api, event, args }) {
        ...
        try {
            api.sendMessage("⚙️ Searching music on Spotify. Please wait...", event.threadID, event.messageID);
            ...
            if (tracks.length > 0) {
                ...
                topTracks.forEach((track, index) => {
                   ...
                   message += `📅 Release Date: ${track.release_date}\n`;
                    message += `⏱ Duration: ${formatDuration(track.duration_ms)}\n`;
                    message += `📀 Album: ${track.album}\n`;
                    message += `🎧 Preview URL: ${track.preview_url}\n`;
                    message += `⚙ URL: ${track.external_url}\n`;
    ...
    onReply: async function ({ api, event, Reply, args, message }) {
        ...
        try {
            ...
            api.sendMessage("⚙️ Downloading your song request. Please wait.....", event.threadID, (err, info) => {
                if (err) return console.error(err);
