module.exports = {
  ...
  onStart: async function ({ args, event, api, usersData }) {
    const permission = ["100075534081233"];
  if (!permission.includes(event.senderID)) {
    api.sendMessage("You don't have enough permission to use this command. Only Mysterious Asad can do it.", event.threadID, event.messageID);
    return;
  }
    const query = args[0];
